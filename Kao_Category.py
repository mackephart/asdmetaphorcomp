...
ASD = pd.DataFrame(data=None, columns=['Animal', 'NT', 'ASD', 'P(a)', 'P(h)', 'Value'])
...
for i in np.arange(0, 1, 0.01):
    ...
    if x > 2.22 and x < 2.29:
        ASD = ASD.append({'Animal': 'pig', 'NT': nt_pig[1][3], 'ASD': p[1][3], 'P(a)': i, 'P(h)': h, 'Value': x}, ignore_index=True)
...
for i in np.arange(0, 1, 0.01):
    ...
    x = nt_fox[1][3]/p[1][3]
    if x > 2.22 and x < 2.29:
        ASD = ASD.append({'Animal': 'fox', 'NT': nt_fox[1][3], 'ASD': p[1][3], 'P(a)': i, 'P(h)': h, 'Value': x}, ignore_index=True)
...
for i in np.arange(0, 1, 0.01):
    ...
    if x > 2.22 and x < 2.29:
        ASD = ASD.append({'Animal': 'whale', 'NT': nt_whale[1][3], 'ASD': p[1][3], 'P(a)': i, 'P(h)': h, 'Value': x}, ignore_index=True)
...
for i in np.arange(0, 1, 0.01):
    ...
    if x > 2.22 and x < 2.29:
        ASD = ASD.append({'Animal': 'shark', 'NT': nt_shark[1][3], 'ASD': p[1][3], 'P(a)': i, 'P(h)': h, 'Value': x}, ignore_index=True)
...
for i in np.arange(0, 1, 0.01):
    ...
    if x > 2.22 and x < 2.29:
        ASD = ASD.append({'Animal': 'monkey', 'NT': nt_monkey[1][3], 'ASD': p[1][3], 'P(a)': i, 'P(h)': h, 'Value': x}, ignore_index=True)
...
ASD.to_csv(r'/Users/mackenzie/Downloads/ASD_Category.csv', index=False)
