...
ASD_Both = pd.DataFrame(data=None, columns=['Animal', 'NT', 'ASD', 'P(a)', 'P(h)', 'Lambda', 'Value'])
...
for i in np.arange(0, 500, 0.1):
    ...
    if x > 2.22 and x < 2.29:
        ASD_Both = ASD_Both.append({'Animal': 'pig', 'NT': nt_pig[1][3], 'ASD': p[1][3], 'P(a)': 0.5, 'P(h)': 0.5, 'Lambda': i, 'Value': x}, ignore_index=True)
...
for i in np.arange(0, 500, 0.1):
    ...
    x = nt_fox[1][3]/p[1][3]
    if x > 2.22 and x < 2.29:
        ASD_Both = ASD_Both.append({'Animal': 'fox', 'NT': nt_fox[1][3], 'ASD': p[1][3], 'P(a)': 0.5, 'P(h)': 0.5, 'Lambda': i, 'Value': x}, ignore_index=True)
...
for i in np.arange(0, 500, 0.1):
    ...
    if x > 2.22 and x < 2.29:
        ASD_Both = ASD_Both.append({'Animal': 'whale', 'NT': nt_whale[1][3], 'ASD': p[1][3], 'P(a)': 0.5, 'P(h)': 0.5, 'Lambda': i, 'Value': x}, ignore_index=True)
...
for i in np.arange(0, 500, 0.1):
    ...
    if x > 2.22 and x < 2.29:
        ASD_Both = ASD_Both.append({'Animal': 'shark', 'NT': nt_shark[1][3], 'ASD': p[1][3], 'P(a)': 0.5, 'P(h)': 0.5, 'Lambda': i, 'Value': x}, ignore_index=True)
...
for i in np.arange(0, 500, 0.1):
    ...
    if x > 2.22 and x < 2.29:
        ASD_Both = ASD_Both.append({'Animal': 'monkey', 'NT': nt_monkey[1][3], 'ASD': p[1][3], 'P(a)': 0.5, 'P(h)': 0.5, 'Lambda': i, 'Value': x}, ignore_index=True)

ASD_Both.to_csv(r'/Users/macke/Downloads/ASD_Both.csv', index=False)
